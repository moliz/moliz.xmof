...
operation StateSystem getMarkingStates() : Set {
	var markingStates : Set = new Set();
	markingStates.addAll(self.getStatesAfterActivityExecution("petrinetConfiguration.NetConfiguration.initializeMarking"));
	markingStates.addAll(self.getStatesAfterActivityExecution("petrinetConfiguration.TransitionConfiguration.fire"));	
...
rule MatchPlaceConfiguration
	...
	extends MatchPlace {
		compare : left.heldTokens.size() = right.heldTokens.size()
	}
...
operation PlaceConfiguration debugPrintln() {
	if(self.heldTokens.size() > 0)
		(self.name + "=" + self.heldTokens.size()).println();
}

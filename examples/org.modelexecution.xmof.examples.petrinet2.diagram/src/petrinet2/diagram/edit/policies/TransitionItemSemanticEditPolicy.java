...
public class TransitionItemSemanticEditPolicy extends
	...
	public TransitionItemSemanticEditPolicy() {
		super(Petrinet2ElementTypes.Transition_2001);
	}
	...
	protected Command getDestroyElementCommand(DestroyElementRequest req) {
		...
		for (Iterator<?> it = view.getSourceEdges().iterator(); it.hasNext();) {
			...
			if (Petrinet2VisualIDRegistry.getVisualID(outgoingLink) == TransitionOutputEditPart.VISUAL_ID) {
			...
			if (Petrinet2VisualIDRegistry.getVisualID(outgoingLink) == TransitionInputEditPart.VISUAL_ID) {
				DestroyReferenceRequest r = new DestroyReferenceRequest(
						outgoingLink.getSource().getElement(), null,
						outgoingLink.getTarget().getElement(), false);
				cmd.add(new DestroyReferenceCommand(r));

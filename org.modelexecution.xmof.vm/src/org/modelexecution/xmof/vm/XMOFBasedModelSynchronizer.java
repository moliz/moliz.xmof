...
public class XMOFBasedModelSynchronizer implements ExecutionEventListener {
	...
	private void handleLinkCreation(Link link) {
		...
		Property referencedProperty = getReferencedProperty(link.type, referencingProperty);
		...
		Command cmd = null;
		if(eReference.isContainment() && referencedEObject.eContainer() == null) {
			cmd = new RemoveCommand(editingDomain, getModelResource().getContents(), referencedEObject);
			execute(cmd);
		}
		if(eReference.isMany()) {
			if(index != -1) {
				cmd = new AddCommand(editingDomain, referencingEObject, eReference, referencedEObject, index);
			} else {
				cmd = new AddCommand(editingDomain, referencingEObject, eReference, referencedEObject);
			}
		} else {
			cmd = new SetCommand(editingDomain, referencingEObject, eReference, referencedEObject);
		}
		execute(cmd);		
	}	
